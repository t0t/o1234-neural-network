...
function makeDraggable(element) {
  ...
  function startDrag(evt) {
    if (evt.target.classList.contains('text-style') || 
        evt.target.classList.contains('circle-style') || 
        evt.target.closest('.connection-group')) {
      
      if (evt.target.closest('.connection-group')) {
        selectedElement = evt.target.closest('.connection-group');
        const mousePos = getMousePosition(evt);
        const line = selectedElement.querySelector('path');
        const pathD = line.getAttribute('d');
        const firstPoint = pathD.split(' ')[0].replace('M','').split(',');
        offset = {
          x: mousePos.x - parseFloat(firstPoint[0]),
          y: mousePos.y - parseFloat(firstPoint[1])
        };
      } else {
        selectedElement = evt.target;
        const mousePos = getMousePosition(evt);
        
        if (selectedElement.classList.contains('circle-style')) {
          offset = {
            x: mousePos.x - parseFloat(selectedElement.getAttribute('cx')),
            y: mousePos.y - parseFloat(selectedElement.getAttribute('cy'))
          };
        } else {
          offset = {
            x: mousePos.x - parseFloat(selectedElement.getAttribute('x')),
            y: mousePos.y - parseFloat(selectedElement.getAttribute('y'))
          };
        }
      }
    }
  ...
  function drag(evt) {
    if (selectedElement) {
      ...
      else if (selectedElement.classList.contains('connection-group')) {
        selectedElement.querySelectorAll('path').forEach(path => {
          const d = path.getAttribute('d');
          const points = d.split(/[ML]/).filter(p => p.length);
          ...
          path.setAttribute('d', 'M' + movedPoints[0] + (movedPoints.length > 1 ? 'L' + movedPoints.slice(1).join('L') : ''));
        });
      }
      else {
